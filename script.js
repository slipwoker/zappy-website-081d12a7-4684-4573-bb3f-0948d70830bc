    ...
    contactForm.addEventListener('blur', function(e) {
      ...
      if (field.tagName === 'INPUT' || field.tagName === 'TEXTAREA') {
        if (field.hasAttribute('required') && field.value.trim()) {
          ...
          const errorMsg = field.parentNode ? field.parentNode.querySelector('.error-message') : null;
          if (errorMsg) {
            errorMsg.remove();
          }
        }
    ...
    "translations": {
      "he": {
        "consentModal": {
          ...
          "description": "יעל שניידר משתמש בעוגיות כדי לשפר את החוויה שלך, לנתח שימוש באתר ולסייע במאמצי השיווק שלנו.",
    ...
    contactForm.addEventListener('submit', async function(e) {
        ...
        try {
            ...
            const response = await fetch('https://api.zappy5.com/api/email/contact-form', {
                ...
                body: JSON.stringify({
                    websiteId: '081d12a7-4684-4573-bb3f-0948d70830bc',
                    name: data.name || '',
                    email: data.email || '',
